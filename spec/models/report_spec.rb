 ...
 RSpec.describe Report, :type => :model do
   ...
   context 'when there is a self evaluation' do
     let(:question) { FactoryGirl.create(:question, description: "Favorite Movie") }
     let!(:answer) { FactoryGirl.create(:answer, question: question, evaluation: participant.self_evaluation) }
  ...
  context 'when there is a peer evaluation' do
    ...
    before do
      5.times do |i|
        self_eval_question = FactoryGirl.create(:question)
        participant.self_evaluation.questions << self_eval_question
        FactoryGirl.create(:answer, evaluation: evaluation_1, numeric_response: 1+i, question: self_eval_question)
      end
    end
